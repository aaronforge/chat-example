// 네임스페이스
export const WS_NAMESPACE_CHAT = '/chat';

// 이벤트
export const WS_EVENT_ROOM_JOIN = 'room.join';
export const WS_EVENT_ROOM_LEAVE = 'room.leave';
export const WS_EVENT_ROOM_EXIT = 'room.exit';
export const WS_EVENT_ROOM_READ = 'room.read';
export const WS_EVENT_ROOM_INVITE = 'room.invite';
export const WS_EVENT_ROOM_TYPING_START = 'room.typing.start';
export const WS_EVENT_ROOM_TYPING_STOP = 'room.typing.stop';
export const WS_EVENT_MESSAGE_SEND = 'message.send';
export const WS_EVENT_MESSAGE_DELETE = 'message.delete';
